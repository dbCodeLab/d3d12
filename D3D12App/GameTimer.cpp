...
{
	return (float)mDeltaTime * mSimulationSpeed;
}
...
void GameTimer::Tick()
{
	...
	if (abs(deltaTime - mDeltaTime) < 1.0f) {
		// Wrap FIFO frame time buffer.
		memmove(&mFrameTime[1], mFrameTime, (MAX_SAMPLE_COUNT - 1) * sizeof(double));
		mFrameTime[0] = deltaTime;
	...
	if (mSampleCount > 0)
	{
		for (ULONG i = 0; i < mSampleCount; i++) mDeltaTime += mFrameTime[i];
